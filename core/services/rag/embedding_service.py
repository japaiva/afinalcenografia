...
class EmbeddingService:
    ...
    def __init__(self, agent_name='Assistente RAG de Feiras'):
        ...
        """
        logger.debug(f"Inicializando EmbeddingService")
        ...
        try:
            if hasattr(self.client, 'chat'):  # OpenAI
                ...
                # Fallback para o Assistente de Briefing
        ...
        try:
            ...
            # Log resumido de config
            logger.info(f"EmbeddingService iniciado: modelo={self.embedding_model}, provider={self.embedding_provider}")
        except Exception as e:
            ...
            logger.info("Usando configuração padrão para embeddings")
    ...
    def _get_param_value(self, nome, categoria, default):
        ...
        return ParametroManager.get_param_value(nome, categoria, default)
    ...
    def gerar_embeddings_qa(self, qa: FeiraManualQA) -> Optional[List[float]]:
        ...
        """
        logger.debug(f"Gerando embedding para QA {qa.id}")
                ...
                try:
                    ...
                    if self.embedding_provider == 'openai':
                        ...
                        if not embedding_client:
                            logger.error(f"Falha ao criar cliente. Tentativa {retry+1}")
                            continue
                        ...
                        try:
                            ...
                            logger.debug(f"Embedding gerado para QA {qa.id}")
                            return embedding
                        except Exception as e:
                            # Tentar com modelo de fallback
                            fallback_model = "text-embedding-ada-002"
                            logger.warning(f"Erro com modelo {self.embedding_model}: {str(e)}. Usando fallback.")
                        ...
                        logger.warning(f"Provider {self.embedding_provider} não suportado. Usando OpenAI como fallback.")
                        embedding_client = self._get_dedicated_openai_client()
                        if not embedding_client:
                            logger.error(f"Falha ao criar cliente para fallback. Tentativa {retry+1}")
                            continue
    ...
    def gerar_embedding_consulta(self, query: str) -> Optional[List[float]]:
        ...
        Returns:
            Lista de floats representando o embedding ou None em caso de erro.
        """
        logger.debug(f"Gerando embedding para consulta")
            ...
            for retry in range(max_retries):
                try:
                    if self.embedding_provider == 'openai':
                        ...
                        if not embedding_client:
                            logger.error(f"Falha ao criar cliente para consulta. Tentativa {retry+1}")
                            continue
                    ...
                    logger.warning(f"Tentativa {retry+1} falhou: {str(e)}")
